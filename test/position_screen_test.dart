  ...
  group('UserLocationPage Tests', () {
    testWidgets('Should display initial UI elements',
      ...
      expect(find.byType(ElevatedButton),
          findsNWidgets(2)); // Assurez-vous d'avoir deux boutons maintenant
      expect(find.byType(ListView), findsOneWidget);
    });

    testWidgets('Navigation to PickupDateScreen works',
        (WidgetTester tester) async {
      await tester.pumpWidget(const MaterialApp(home: UserLocationPage()));

      // Simulez l'appui sur le bouton 'Valider'
      await tester.tap(find.text('Valider'));
      await tester
          .pumpAndSettle(); // Attendre que toutes les animations se terminent
